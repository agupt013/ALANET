...
def create_clips(root, destination, garbage):
    """
    ...
    for file in files:
        ...
        if len(images) < 29:
            move(os.path.join(root, file), os.path.join(garbage, file))
            continue

        for imageCounter, image in enumerate(images):
            # Bunch images in groups of 27 frames
            if (imageCounter % 29 == 0):
                if (imageCounter + 28 >= len(images)):
                    break
                folderCounter += 1
...
def main():
    ...
    garbagePath      = os.path.join(args.dataset_folder, "garbage")
    os.mkdir(extractPath)
    os.mkdir(trainPath)
    os.mkdir(testPath)
    os.mkdir(validationPath)
    os.mkdir(garbagePath)
    ...
    if(args.dataset == "adobe240fps"):
        ...
        videos = f.read().split('\n')
        extract_frames(videos, args.videos_folder, extractPath)
        create_clips(extractPath, trainPath, garbagePath)
        ...
        create_clips(extractPath, testPath, garbagePath)
